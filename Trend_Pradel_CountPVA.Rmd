...
CJS_asmi %>%
  ...
  write.table(file = "data/CJS_asmi_mark.inp", sep = " ", 
...
CJS_asmi %>%
  ...
  write.table(file = "data/CJS_asmi_mark_site.inp", sep = " ", 
              col.names = FALSE, row.names = FALSE)
...
## Convert for MARK  
asmi.inp <- convert.inp("data/CJS_asmi_mark.inp", 
                        group.df = plotdf,
                        covariates = NULL,
                        use.comments = FALSE)

asmiSite.inp <- convert.inp("data/CJS_asmi_mark_site.inp", 
                        group.df = sitedf,
                        covariates = NULL,
                        use.comments = FALSE)
```
...
CJS   
missed surveys: index and fix to zero detection
```{r}
...
Takes a long time, can skip and load in the next chunk
```{r}
...
Pradel_asmi %>%
  ...
  bind_cols(dmPlot) %>%
  mutate(Plot799 = paste(Plot799, ";", sep = "")) %>%
  write.table(file = "data/Pradel_asmi_mark.inp", sep = " ", 
              col.names = FALSE, row.names = FALSE)


asmiPradel.inp <- convert.inp("data/Pradel_asmi_mark.inp", 
...
asmi.models <- run.asmi()
save(asmi.models, file = "data/PradelSBC.Rdata")
```
